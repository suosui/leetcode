...
var findSubstring = function (s, words) {
    const wordLength = words.length * words[0].length;
    const res = [];
    const wordJoined = words.sort().join('');
    for (let i = 0; i < s.length - wordLength + 1; i++) {
        const strChunks = chunk(s.slice(i, i + wordLength), words[0].length);
        const strJoined = strChunks.sort().join('');
        if (strJoined === wordJoined) {
            res.push(i);
        }
    }
    return res;
